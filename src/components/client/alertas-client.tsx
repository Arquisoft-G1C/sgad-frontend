...
export function AlertasClient({ initialAlerts }: AlertasClientProps) {
    ...
    const filteredAlerts = alerts.filter((alert: any) => {
        ...
        if (filters.severity && filters.severity !== "all" && alert.priority !== filters.severity) return false
        if (filters.status && filters.status !== "all" && alert.status !== filters.status) return false
        return true
    })

    const activeAlerts = filteredAlerts.filter((alert: any) => alert.status === "active")
    const highPriorityAlerts = activeAlerts.filter((alert: any) => alert.priority === "high")
